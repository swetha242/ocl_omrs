...
Separate files should be created for concepts and mappings, for example:
    ...
    manage.py extract_db --org_id=CIEL --source_id=CIEL --raw -v0 --sources > sources.json
    manage.py extract_db --org_id=CIEL --source_id=CIEL --raw -v0 --classes > classes.json
...
from omrs.models import Concept, ConceptReferenceSource ,ConceptClass
...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        ...
        make_option('--token',
                    ...
                    dest='token',
                    default=None,
                    help='OCL API token to validate OpenM...................................................................................................................................................................................................................................................................................................................................................................................................................................................................................................................RS reference sources'),
    )
    ...
    def handle(self, *args, **options):
        ...
        if self.do_mapping or self.do_concept or self.do_retire or self.do_source or self.do_class:
            self.do_export = True
    ...
    def print_debug_summary(self):
        ...
        if self.do_source:
            print 'EXPORT COUNT: Total Sources exported: %d' % self.cnt_sources_exported
        if self.do_class:
            print 'EXPORT COUNT: Total Classes exported: %d' % self.cnt_classes_exported
        print '------------------------------------------------------'
    ...
    def export(self):
        ...
        if self.do_source:
            source=ConceptReferenceSource.objects.all()
            sources_enum=enumerate(source)
            export_data1=''
            for num,src in sources_enum:
                self.cnt_sources_exported+=1
                export_data1=self.export_source(src)
                if export_data1:
                    print json.dumps(export_data1, indent=output_indent)
        if self.do_class:
            classes = ConceptClass.objects.all()
            classes_enum = enumerate(classes)
            export_data1 = ''
            for num, cls in classes_enum:
                 self.cnt_classes_exported += 1
                 export_data1 = self.export_class(cls)
                 if export_data1:
                     print json.dumps(export_data1, indent=output_indent)
    ...
    def export_source(self,source):
        ...
        data['creator']=source.creator
        #data['date_created']=source.date_created
        data['retired']=source.retired
        data['retired_by']=source.retired_by
        data['uuid']=source.uuid
        add_f(data,'hl7',source.hl7_code)
        add_f(data, 'date_retired', source.date_retired)
        add_f(data, 'retire_reason', source.retire_reason)
    ...
    def export_class(self, cls):
            data = {}
            data['concept_class_id'] = cls.concept_source_id
            data['name'] = source.name
            data['description'] = source.description
            data['creator'] = source.creator
            # data['date_created']=source.date_created
        ...
        for ref_map in concept.conceptreferencemap_set.all():
            ...
            if ref_map.concept_reference_term.concept_source.name == self.org_id:
                ...
                map_dict = self.generate_internal_mapping(
                    ...
                    external_id=ref_map.concept_reference_term.uuid,ref_m=ref_map.uuid)
            ...
            # External Mapping
                ...
                map_dict = self.generate_external_mapping(
                    ...
                    ref_term=ref_map.concept_reference_term.uuid)
    ...
    def export_concept_qanda(self, concept):
        ...
        for answer in concept.question_answer.all():
            map_dict = self.generate_internal_mapping(
                ...
                external_id=answer.uuid,sort_weight=answer.sort_weight)
    ...
    def export_concept_set_members(self, concept):
        ...
        for set_member in concept.conceptset_set.all():
            map_dict = self.generate_internal_mapping(
                ...
                external_id=set_member.uuid,sort_weight=set_member.sort_weight)
            maps.append(map_dict)
    ...
    def generate_internal_mapping(self, map_type=None, from_concept=None,
                                  ...
                                  sort_weight=None,retired=False,ref_m=None):
	...
	map_dict['ref_m']=ref_m
        add_f(map_dict, 'external_id', external_id)
	add_f(map_dict, 'sort_weight', sort_weight)
        return map_dict
    ...
    def generate_external_mapping(self, map_type=None, from_concept=None,
                                  ...
                                  external_id=None, retired=False,ref_term=None):
        ...
        add_f(map_dict, 'external_id', external_id)
        add_f(map_dict, 'ref_term', ref_term)
        return map_dict
